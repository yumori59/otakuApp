...
| 1-11 | AdMob 組み込み | SDK導入、バナー（一覧下部）、ネイティブ広告（申込一覧の6件目）、リワード広告。**当落確認の直前後に出さない**制御ロジック、ATT（App Tracking Transparency）ダイアログ、初期化の非同期化 | **4.0** |
...
| T7 | AdMob SDK が起動時間を悪化させる / 審査で問題になる | 低 | 起動が遅くなり離脱。ATT関連のリジェクト | Phase 1 の 1-11 | 初期化を非同期化。ATTダイアログの文面を審査ガイドラインに沿わせる。起動TTI p95 < 2秒を計測 | 起動時間の計測 |
