...
| R1-7 | 登録済みの名義（表示名・続柄・色・入会日・メモ）とFC会員情報（FC名・会員番号・更新日・年会費）を後から編集できる。単体削除もでき、名義削除は配下の会員情報も連鎖削除する（申込記録自体は残る） | モック未実装（**追加要件**。`docs/plans/identity-edit-and-delete/`） |
| R1-8 | 名義一覧で FC / アーティスト名ごとに名義を束ねて俯瞰できる（1名義が複数FCに所属する場合は各グループに重複表示） | `screenIdentities` の FC別グルーピング（2026-08-20 追加・0-4b） |
...
| R2-9 | 登録済みの申込（代表者・同行者・公演情報・ツアー名・ステータス・座席・メモ）を後から編集できる | モック未実装（**追加要件**。`docs/plans/application-edit/`） |
...
| S2 | 名義一覧 | タブ2 | FC名でグルーピング表示、並び替え（更新が近い順 / 当選が多い順 / 入会が古い順）は**グループ内**の順序、名義行 | `screenIdentities` |
...
| データ保全 | ユーザーデータの喪失をゼロに。削除は全てソフトデリート＋復元可能期間30日。**復元UI・復元APIは未実装**（2026-08-20時点。`deleted_at` によるソフトデリートのみ実装済みで、取り消す画面・操作は無い。既知の乖離として `docs/plans/delete-ui/plan.md` リスク R-1 に記録） |
| セキュリティ | 共有はトークンベースで失効可能。詳細は [08-compliance-risk.md](./08-compliance-risk.md) |
...
| C3 | ~~会員番号を**暗号化保存 + 既定で下4桁表示**に変更~~ **2026-08-20 撤回**。理由: 利用者が自分の会員番号を確認できないため。全桁を平文で保存・常時表示する（[08](./08-compliance-risk.md) R20） |
