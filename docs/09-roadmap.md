...
| 0-6b | 会員情報編集・削除（0-7の拡張） | 会員情報カードをタップ→FC名・会員番号下4桁・更新日・年会費を編集（`MembershipFormView(mode: .edit)`、通知許可シートは出さない）。編集シート最下部に単体削除ボタン。削除しても名義・申込は残る。`docs/plans/identity-edit-and-delete/` | **1.5** |
...
| 0-11c | ツアー編集・削除（0-9の拡張） | 申込タブ→ツアー表のヘッダーに「編集」「削除」を追加（専用画面は新設しない）。編集はツアー名・アーティスト名の2項目を持つ専用シート（`TourFormView(mode: .edit)`、`AppSheet.editTour(id:)`）。削除は`confirmationDialog`に配下の公演N件・申込M件の実数を表示し、共有中なら警告行を追加。削除は配下 events/applications/application_companions までソフトデリートを連鎖させる（identity/membershipは連鎖削除の対象外）。書き込みは`POST /v1/sync/push`のみ（BE `PATCH/DELETE /v1/tours/:id`は未使用）。`docs/plans/tour-edit-and-delete/` | **1.5** |
