...
struct ShareRecipientsView: View {
    ...
    /// **0 件では発行させない**（招待はアクセス制限そのもの — `api-contract-delta.md` §1）
    ...
    /// 追加・削除は `ShareLinkStore.addRecipients` / `removeRecipient` を叩き、
    /// 手元の `link.recipients`（= `shareLinks.links` 由来）がそのまま新しい一覧になる。
    @ViewBuilder
