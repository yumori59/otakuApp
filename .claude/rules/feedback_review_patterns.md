...
| BE-11 | **単一トランザクション内で per-item try/catch → reject を返す設計は、DB 例外が起きると accepted が嘘になる** | Postgres はトランザクション内で1クエリでもエラーになると abort 状態になり、以後 COMMIT しても暗黙 ROLLBACK される。`$transaction` 内でmutationごとに try/catch して個別に `rejected` へ積むだけの実装だと、**先に成功したはずの mutation もロールバックされて消えるのに、レスポンスは `accepted` のまま**という虚偽成功応答になる（`sync.service.ts`のpushで実測。同一バッチで正当な名義作成→ツアー名unique違反の順に送ると、名義がDBに保存されないのに`accepted`に含まれた。2026-08-18）。対策は二段構え: ①想定される例外（unique違反等）は**事前チェックで発生自体を防ぐ**（`tx`で`findFirst`してからupsert） ②予期しないDB例外（`AppError`以外）は握り潰さず**rethrowしてpush全体を失敗させる**（outboxは何もmarkSyncedされないので安全に再送できる。個別rejectで返すのは、ビジネスロジック起因の`AppError`だけに限定する） |
