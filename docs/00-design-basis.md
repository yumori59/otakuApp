...
| 会員番号 | **全桁を平文で保存し常時表示**（2026-08-20 ユーザー判断で暗号化・下4桁表示を撤回）。共有時のマスキングは維持 |
...
| **他人の個人情報（氏名・FC会員番号）の預託** | 個人情報保護法上の安全管理措置義務、漏洩時の重大な信用失墜 | 共有時マスキング既定、保存しない選択肢の提供（`08`）。**会員番号の暗号化・下4桁表示は 2026-08-20 ユーザー判断により撤回**（残存リスクは `08` R20 として受容） |
