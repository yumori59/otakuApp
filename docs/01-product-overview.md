...
| R1-7 | 登録済みの名義（表示名・続柄・色・入会日・メモ）とFC会員情報（FC名・会員番号下4桁・更新日・年会費）を後から編集できる。単体削除もでき、名義削除は配下の会員情報も連鎖削除する（申込記録自体は残る） | モック未実装（**追加要件**。`docs/plans/identity-edit-and-delete/`） |
