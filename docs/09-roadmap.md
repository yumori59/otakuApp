...
|---|---|---|---|---|
| **Phase 0**<br>ローカル完結MVP | **「毎日開く理由」があるかを、0円で検証する** | 名義CRUD、FC会員情報CRUD、申込CRUD、ツアー表、ローカル通知、ホームサマリ、推しカラー | ・リリース8週後の **D30継続率 ≥ 20%**<br>・**週次記録率 ≥ 40%**（週に1回以上、申込か当落を記録したMAU比）<br>・**名義2件以上の登録率 ≥ 50%**<br>・ストアレビュー **平均 ≥ 4.0**（10件以上）<br>・クラッシュ率 **< 0.5%** | **¥0**<br>(Apple ≈ ¥1,279のみ) |
| **Phase 1**<br>共有と収益化 | **「他人を連れてくる導線」と「課金」を成立させる** | NestJS API（Cloud Run）、Supabase Postgres、Sign in with Apple、複数端末同期、共有リンク（**アプリ内 SharedBoard**）、統計、Plus サブスク、AdMob | ・**MAU 3,000**<br>・**課金率 ≥ 1.0%**（MAU比）<br>・**MRR ≥ ¥30,000**（Free 立ち上げ ≈¥1,500 の約20倍／Pro 移行後 ≈¥5,500 の約5倍）<br>・~~共有リンク経由の新規インストール比率 ≥ 10%~~ **[要再設定]**<sup>†</sup><br>・同期失敗率 **< 1%** | **≈ ¥1,300〜1,700**<br>（Free 立ち上げ） |
| **Phase 2**<br>共同編集と定着 | **「グループで使うツール」にして解約率を下げる** | 共同編集ボード、ウィジェット、Live Activity、エクスポート、統計強化、iPad対応 | ・**MAU 15,000**<br>・**MRR ≥ ¥300,000**<br>・月次解約率 **< 5%**<br>・共同編集ボードの利用率 **≥ 25%**（Plus会員比）<br>・~~共有リンク経由の新規比率 ≥ 20%~~ **[要再設定]**<sup>†</sup> | **≈ ¥7,500〜10,800**<br>（MAU 1万・Pro 帯） |

<sup>†</sup> **残課題（2026-08-07・未着手）**: `docs/plans/share-account-invites/` にて共有をアカウント招待制へ完全移行することが決定（Q1=A）し、
公開経路 `GET/PATCH /public/shares/:token` が廃止された。招待されたアカウントしか共有を開けなくなったため、
「未導入ユーザーがリンクを踏んでインストールする」という導線自体が消滅し、上記2つの KPI は**現行仕様では達成不能**になった。
KPI の再設定（削除するか、招待経由の別指標に置き換えるか）はロードマップ側の意思決定として**未着手**。
本計画（share-account-invites）はスコープ外として記録のみ行い、決定は行っていない（`plan.md` §1.1）。
...
| 0-4b | 名義一覧のFC別グルーピング | `fanClubNameRaw` をキーに名義を FC 別に束ねて表示。複数FC所属は全グループに重複表示。ソートはグループ内の並び順 | **1.0** |
...
| 0-5b | 名義編集・削除（0-5の拡張） | 名義詳細右上「編集」→表示名・続柄・色・入会日・メモを編集（`IdentityFormView(mode: .edit)`）。本文最下部に削除ボタン（`confirmationDialog`）。削除は配下の会員情報も連鎖削除し、それらを代表会員情報にしていた申込は`repMembershipID`をnullにクリア（申込自体は残す）。削除済み名義を参照する申込は「削除された名義」と非リンク表示。`docs/plans/identity-edit-and-delete/` + `docs/plans/delete-ui/` | **2.0** |
| 0-6 | 名義追加フォーム | 氏名・関係性（4択）・入会日・推しカラー・メモ。保存後に会員情報登録へ連続遷移するフロー | **2.0** |
| 0-6b | 会員情報編集・削除（0-7の拡張） | 会員情報カードをタップ→FC名・会員番号・更新日・年会費を編集（`MembershipFormView(mode: .edit)`、通知許可シートは出さない）。編集シート最下部に単体削除ボタン。削除しても名義・申込は残る。`docs/plans/identity-edit-and-delete/` | **1.5** |
...
| 0-11b | 申込編集（0-11の拡張） | 0-11で作成した申込を後から編集する画面。追加フォームと同一12項目 + ツアー付け替え。公演情報の変更は同一公演を参照する他申込へ波及する旨を保存前に明示。詳細（0-10）に編集導線を追加。`docs/plans/application-edit/` | **1.5** |
...
| ~~0-13~~ | ~~会員番号の暗号化保存~~ | ~~CryptoKit（AES-GCM）＋ Keychain（iCloud Keychain同期）での鍵管理。既定表示は下4桁のみ、タップで全表示、「保存しない」選択肢~~ **2026-08-20 撤回**（ユーザー判断。会員番号は全桁を平文で保存・常時表示する。`docs/plans/membership-full-number/`） | ~~2.0~~ **0** |
...
| 0-17 | ユニットテスト | 日付計算（`daysUntil` / `renewalBadge` の境界値: -1日/0日/14日/15日/30日/31日）、ツアーグルーピング、通知スケジュール、ソート3種。**カバレッジ目標60%**（~~暗号化の往復~~ は 0-13 撤回に伴い対象外） | **3.0** |
...
| | **Phase 0 合計** | ※0-13 撤回（−2.0）反映後 | **55.5** |

> **削除の復元・取り消しUIは未着手**（2026-08-20）。0-5b/0-6b の名義・会員情報削除、0-10 の申込削除（`docs/plans/delete-ui/`）は
> いずれも `deleted_at` によるソフトデリートのみを実装し、削除を取り消す画面・操作は存在しない。
> [`./01-product-overview.md`](./01-product-overview.md) `:250` の「復元可能期間30日」との乖離であり、`docs/plans/delete-ui/plan.md` のリスク R-1 として認識済み。着手Phaseは未確定。
...
| 1-6 | 共有リンク発行（**2026-08-07 アカウント招待制へ再定義・実装済み**） | `share_links` + `share_recipients`（ACL の実体）テーブル、NestJS によるトークン発行（128bitランダム・有効期限30日）、名義単位・ツアー単位の2種、招待先ACC-ID 1〜20件必須、招待の追加削除、失効操作。詳細 `docs/plans/share-account-invites/` | **3.0** |
| 1-7 | ~~Next.js 共有Webビュー~~ → **廃止（2026-08-05）**。**さらに 2026-08-07 に `GET/PATCH /public/shares/:token` 自体を廃止** | 独立 Web は作らない。閲覧・編集は iOS `SharedBoard` + Bearer 必須の受信箱 `/v1/shares/received/*`（実装済み）。未ログイン閲覧・Universal Links による外部公開は非対象になった | **0** |
| 1-8 | 共有プレビュー画面と公開設定（**2026-08-07 招待制UIへ再定義・実装済み**） | モック `screenSharePreview` の実装。名義ごとの `historyVisible` トグル、招待先ACC-ID入力（必須・実在確認）、リンクコピー、共有シート連携。受け取り側は `SharedInbox`（受信箱）+ `SharedBoard` | **3.0** |
...
> **2026-08-20 撤回**: 「会員番号の暗号化保存」（旧 `p13`, 6d）はユーザー判断により撤回し、工数を 0 とした（2.1 の表参照）。上記ガントからはタスクを削除し、後続の `p15` は `p12` の直後に付け替えている。
...
| L2 | 他人の個人情報（氏名・FC会員番号）の預託 | 中 | 個人情報保護法上の安全管理措置義務違反、漏洩時の重大な信用失墜 | ~~Phase 0 の 0-13~~ **0-13 は 2026-08-20 撤回** | ・~~会員番号はクライアント側暗号化（サーバーは復号不可）~~ ・~~既定表示は下4桁~~ → **撤回。会員番号は全桁を平文で保存・表示**（残存リスクは `08` R20 として受容）<br>・共有時はマスキング既定<br>・入力自体は任意（保存しない選択肢）<br>・[`./08-compliance-risk.md`](./08-compliance-risk.md) に安全管理措置を文書化 | 四半期のセキュリティ監査 |
| L3 | 共有リンクのURLが第三者に渡り、他人の情報が漏れる | 低（2026-08-07 招待制移行により大幅低減） | 名義主との信頼関係の破壊（アプリの存在意義の否定） | Phase 1 の 1-6 | ・**招待制（アカウント認可）**: URLを知っているだけでは開けず、`share_recipients` に招待されたアカウントでログインしていないと閲覧・編集できない（`docs/plans/share-account-invites/`）<br>・トークンは128bitランダム<br>・**有効期限30日の既定**<br>・失効操作を1タップで<br>・招待先アカウントの追加削除をオーナーがいつでも行える<br>・共有内容は名義主が確認できるプレビューを必須通過<br>・~~検索エンジンへの`noindex`~~（認証必須ルートになりクロール対象外のため不要） | 共有ビューのアクセスログ（想定外の参照元）、非招待アカウントによる `SHARE_NOT_INVITED`/`SHARE_INVALID` の発生頻度 |
