...
| Q12 | ✅ 提案どおり |
| Q13 | ✅ 提案どおり |
| Q14 | ⏳ **未回答**（Q1=A 確定により新たに発生。planner 提案は 14-a。`plan.md` は 14-a 前提で記述済み） |

**Q1〜Q13 は 2026-08-07 にユーザー回答済み。** `requirements.md` / `api-contract-delta.md` / `plan.md` は確定内容に合わせて更新済み。

**Q14 のみ未回答。** 14-a（提案・記述済みの前提）で進める場合はそのまま着手してよい。
14-b を選ぶ場合は `plan.md` §11 の差分を適用してから着手する。
