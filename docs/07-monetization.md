...
[`./00-design-basis.md`](./00-design-basis.md) の「6. 収益化方針」で決めた骨格（**Free は3名義まで / 申込件数は無制限 / Plus は月¥350・年¥2,800 / 無料版は広告**）を、実装に落とせるレベルまで具体化したものです。決めることは、プラン間の機能差分（＝feature flag 一覧）、ペイウォールの位置と表示コピー、広告の配置と禁止領域、**予想コストと売上・損益試算**、StoreKit 2 / RevenueCat の実装要件（特に **Plus 解約後の名義の扱い**）。
...
```mermaid
flowchart LR
    B[Free で記録開始<br/>名義1〜3件] --> C{名義4件目?}
...
**(c) 無料版でも価値の証明が完結する。** 最大価値である J1（FC更新期限を落とさない）は、名義3件・通知ありの Free で完全に成立します。つまり無料ユーザーは「損な体験」をしていない。これはフリーミアムの成功条件そのもので、「無料版は削って不便にしてある」型よりレビュー評価と口コミが安定します。
...
|---|---|---|---|---|---|
| 1 | 名義の登録・編集・削除 | `screenAddIdentity` / `screenIdentityDetail` | △ 3件 | ○ 無制限 | **唯一の主要な上限** |
| 2 | 関係性・備考メモ | `f-relation` / `saveNote` | ○ | ○ | |
| 3 | FC会員情報の登録 | `screenAddMembership` | ○ 名義あたり無制限 | ○ | 名義数で絞れているので会員情報は絞らない |
| 4 | 会員番号の保存・下4桁表示 | `membership-no` | ○ | ○ | セキュリティ機能は課金対象にしない（`08`） |
...
| # | 利点 | 説明 |
|---|---|---|
| 1 | **ユーザーが価値を認めた「後」の行動である** | 名義を4件目に増やす人は、すでに1〜3件を登録し、通知を受け取り、申込を記録し、「このアプリは機能する」と確認した人。**支払い意思が最大化した瞬間にだけ**ペイウォールが出る。逆に起動時ハードウォールや期間制限トライアルは価値の証明前に判断を強制するので CVR が構造的に低い |
| 2 | **使用量と提供コストが比例する** | 名義が増えれば membership 行・通知スケジュール・同期対象が増える。「たくさん使う人が払う」という説明が成立する |
| 3 | **ヘビーユーザーの定義と一致する** | 名義4件以上＝「自分と家族だけでは足りず友人の名義まで管理している」層。滞在時間・記録件数・継続率すべてが高くLTVも最高。**収益の最大化と体験の最良化が一致する**数少ない設計 |
| 4 | **上限が見えるので不意打ちにならない** | 名義一覧に「3 / 3」と常時表示できるため、「使い始めたら急に金を取られた」型のレビュー低評価を防げる |
...
モックの `identities` は4件（本人・妹・友人2名）で、うち2件は「借りている名義」です。実際のユースケースを写したサンプルが4件であることは、**典型ユーザーが3〜4件に到達する**ことを示しています。

```
名義1件: 自分だけ           → 無料。まだ価値を実感していない
名義2件: 自分＋家族1人      → 無料。ここで通知の価値を実感する
名義3件: 自分＋家族2人      → 無料。「家族で応募」までは完全に無料で完結
──────────── ここに壁 ────────────
名義4件: 友人の名義に手を出した → 課金判定。管理の複雑さが質的に変わる瞬間
```

**閾値3の本質は「自分と家族で完結する範囲は無料、他人を巻き込み始めたら有料」という線引き**です。優れている理由:

1. **3件目までのユーザーは課金しないが、広告を見て、口コミを運び、レビューを書く。** 無料層を狭めるとこの層が消え、アプリが伸びない。
2. **4件目に到達した瞬間、管理の困難さが最大化している。** 他人の名義は「更新期限を落とすと相手に迷惑がかかる」「当落を連絡する義務がある」という質的に重い管理対象で、月¥350の価値がその瞬間に最も高く見える。
3. **モックのサンプル（4件）が壁の1つ先にある。** 実データを写したサンプルが「ちょうど課金判定に触れる」位置にあるのは、閾値がユーザー分布の中央付近にあることを示します。高すぎて誰も当たらない／低すぎて全員が即当たる、のどちらでもない。
...
| **3（既定）** | 家族利用までは無料で完結。到達時点で価値証明が済んでいる | — |
...
**実施条件**: 有意差には各群1,000インストール以上必要。MAU 1,000規模では検定力が足りないので、**Phase 1 は 3 で固定し、MAU 5,000超で A/B を開始**。それ以前はペイウォール到達率の生値だけを見て、10%を大きく下回るなら閾値を下げる方向で単独変更（前後比較）します。
...
| 種別 | 定義 | 使う場所 |
|---|---|---|
| ハードウォール | 課金しないと操作が完了しない | 名義4件目の追加、共有リンク2本目の発行 |
...
#### (A) 4件目の名義追加をタップした瞬間 — ハードウォール

トリガー: 名義が3件のときに名義一覧の `＋` またはホームのアクションシート「名義を追加」をタップ。**フォームを開いてから止めるのではなく、開く前に出します**（入力後に止めるのは最も嫌われる）。
...
報酬は名義上限 +1（3 → 4）、有効期間は視聴完了から **30日**、同時に持てるボーナス枠は **1枠まで**（+2 不可）、再視聴による延長は残り7日以内になってから可、**月間視聴上限は2回**。Plus 契約中は導線を表示しません。
...
-- 名義上限 = Plus契約中 ? 無制限
--          : 3 + (bonus_slots_expires_at > now() ? bonus_identity_slots : 0)
```

**課金への踏み台になる仕組み**

1. 4件目の名義を**実際に作って30日間使える**＝Plus の主機能を無料で体験する。
2. 30日後に上限に戻るとき、4件目は消えず**閲覧のみ**になる（9.5 と同じ挙動）。すでに使っていた名義が読み取り専用になる体験は、初めて壁に当たるより支払い意思が強い。
...
| p | 課金率（MAU比） | **3%** | 一般水準1〜5%。名義4件到達率10%程度の3割が課金する想定 |
...
### 9.5 【最重要】Free に戻ったときの4件目以降の名義の扱い
...
Plus 期限切れ
   ...
   └─ 猶予終了 → ソフトロック期間（14日）
                 ...
                 ・14日間、縮退の予告を表示
              → 縮退の適用（アクティブ枠3件を決定、4件目以降は locked）
```

**アクティブ枠の決定ルール**: ソフトロック期間中に「残す3件を選ぶ」画面を表示します。
...
| 操作 | locked |
|---|---|
| 名義詳細を開く / FC会員情報・更新日・年会費の閲覧 / 会員番号の下4桁表示 / 申込履歴の閲覧 | ○ |
...
1. フリーミアム（サブスク）＋ 無料版広告のハイブリッド。Free = 名義3件 / 申込無制限 / 広告あり、Plus = 月¥350・年¥2,800 / 名義無制限 / 広告なし。
...
4. ペイウォールは4トリガー（名義4件目=ハード / 共有2本目=ハード / 統計=ソフト / 設定=常設）。
...
7. Billing Grace Period 16日 + 独自のソフトロック期間14日。**Free 復帰時、4件目以降の名義は削除しない（閲覧のみ・編集不可・通知は継続）**。
